...
class ParserCSV:
    ...
    def validar_cliente(self, cliente: Dict) -> bool:
        ...
        if not (-90 <= cliente["latitud"] <= 90):
            raise ValueError(
                f"Latitud inválida para cliente {cliente['id']}: {cliente['latitud']}"
            )

        if not (-180 <= cliente["longitud"] <= 180):
            raise ValueError(
                f"Longitud inválida para cliente {cliente['id']}: {cliente['longitud']}"
            )

        # Validar prioridad
        if not (1 <= cliente["prioridad"] <= 5):
            raise ValueError(
                f"Prioridad inválida para cliente {cliente['id']}: {cliente['prioridad']}"
            )

        # Validar pedido
        if cliente["pedido"] <= 0:
            raise ValueError(
                f"Pedido inválido para cliente {cliente['id']}: {cliente['pedido']}"
            )

        # Validar ventana horaria
        try:
            self.validar_ventana_horaria(
                cliente["ventana_inicio"], cliente["ventana_fin"]
            )
        except Exception as e:
            raise ValueError(
                f"Ventana horaria inválida para cliente {cliente['id']}: {str(e)}"
            )
...
5,Cliente E,-12.0264,-77.0628,4,14:00,18:00,125.75"""
    ...
    def validar_vehiculo(self, vehiculo: Dict) -> bool:
        ...
        # Validar capacidad
        if (
            not isinstance(vehiculo["capacidad"], (int, float))
            or vehiculo["capacidad"] <= 0
        ):
...
def cargar_clientes_csv(archivo_csv="Dataset/clientes_lima_1500.csv"):
    """Función auxiliar para cargar clientes desde el archivo CSV por defecto"""
    import os

    if not os.path.exists(archivo_csv):
        raise FileNotFoundError(f"Archivo CSV no encontrado: {archivo_csv}")
