...
class Backtracking:
    ...
    def calcular_cota_inferior(self, ruta_actual: List[str], clientes_restantes: List[Dict], 
                             matriz_distancias: Dict) -> float:
        ...
        if clientes_restantes:
            ...
            if ruta_actual:
                ...
                min_distancia_mst = min(matriz_distancias[ultimo_nodo][nodo] for nodo in nodos_restantes)
                min_distancia_deposito = min(matriz_distancias[nodo]['deposito'] for nodo in nodos_restantes)
                
                return distancia_actual + mst_costo + min_distancia_mst + min_distancia_deposito
    ...
    def verificar_restricciones(self, ruta_actual: List[str], cliente: Dict, 
                              vehiculo: Dict, matriz_distancias: Dict) -> bool:
        """Verifica si se cumplen todas las restricciones"""
        # Verificar capacidad
        carga_actual = sum(c['pedido'] for c in self.obtener_clientes_en_ruta(ruta_actual))
        ...
        if ruta_actual:
            ...
            tiempo_actual = sum(matriz_distancias[ruta_actual[i]][ruta_actual[i + 1]] * 2 
                              for i in range(len(ruta_actual) - 1))
    ...
    def backtracking_rutas(self, ruta_actual: List[str], clientes_restantes: List[Dict], 
                          vehiculos_disponibles: List[Dict], matriz_distancias: Dict, 
                          nivel: int = 0):
        ...
        if not clientes_restantes:
            if ruta_actual:
                ...
                costo_total = sum(matriz_distancias[ruta_completa[i]][ruta_completa[i + 1]] 
        ...
        for i, cliente in enumerate(clientes_restantes):
            ...
            if not self.verificar_restricciones(ruta_actual, cliente, vehiculos_disponibles[0], matriz_distancias):
                continue
            ...
            self.backtracking_rutas(ruta_actual, clientes_restantes_nuevo, 
                                  vehiculos_disponibles, matriz_distancias, nivel + 1)
            
            # Backtrack
            ruta_actual.pop()
    
    def dividir_problema_por_vehiculos(self, clientes: List[Dict], vehiculos: List[Dict]) -> List[Dict]:
        """Divide el problema en subproblemas por vehículo"""
        # Ordenar por prioridad y capacidad
        clientes_ordenados = sorted(clientes, key=lambda x: (x['prioridad'], -x['pedido']))
        ...
        for vehiculo in vehiculos_ordenados:
            ...
            for cliente in clientes_ordenados:
                if (cliente['id'] not in clientes_asignados and 
                    cliente['pedido'] <= capacidad_restante):
            ...
            if clientes_vehiculo:
                asignaciones.append({
                    'vehiculo': vehiculo,
                    'clientes': clientes_vehiculo
                })
        
        return asignaciones
    
    def optimizar_ruta_individual(self, clientes: List[Dict], vehiculo: Dict, 
                                matriz_distancias: Dict) -> Dict:
        ...
        if self.mejor_solucion:
            distancia_total = sum(matriz_distancias[self.mejor_solucion[i]][self.mejor_solucion[i + 1]] 
            ...
            return {
                ...
                'carga_total': sum(c['pedido'] for c in clientes),
                'tiempo_estimado': 0,
                'ruta_optimizada': ['deposito'] + [c['id'] for c in clientes] + ['deposito']
            }
    ...
    def optimizar_rutas(self, clientes: List[Dict], vehiculos: List[Dict]) -> Dict:
        ...
        try:
            ...
            clientes_atendidos = sum(len(ruta['clientes']) for ruta in rutas_optimizadas)
